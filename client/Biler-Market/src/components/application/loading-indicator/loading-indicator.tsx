...
export const LoadingIndicator = ({ type = "line-simple", size = "sm", label }: LoadingIndicatorProps) => {
    const renderSpinner = () => {
        ...
        if (type === "dot-circle") {
            return (
                <svg className={cx("animate-spin text-blue-600", styles[size].spinner)} viewBox="0 0 36 36" fill="none">
                    <path
    ...
    return (
        <div className={cx("flex flex-col items-center justify-center", styles[size].root)}>
            {renderSpinner()}
            {label && <span className={cx("text-gray-600", styles[size].label)}>{label}</span>}
        </div>
    );
};
